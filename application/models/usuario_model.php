...
where users.use_username='" . $username . "'");
    ...
    function save_score()
    {
        $query = $this->db->get_where('use_score', array(
    ...
    public function update_has_need(
        $username,
        $need_vision,
        $need_visiondescri,
        $need_audicion,
        ...
        $need_coginitiva,
        $need_cognitivatexto,
        $need_cognitivainstru,
        $need_cognitivaconcentra
    ) {
        $data = array(
    ...
    public function get_need_voice_recognition($username)
    {
        $this->db->select('use_voice_recognition');
        $this->db->from('use_student');
        $this->db->where('use_username', $username);
        $this->db->limit(1);
        $query = $this->db->get();
    ...
    public function update_user($username)
    {
        ...
        $optVoiceRecognition = $this->input->post('useVoiceRecognition');
        ...
        $data2 = array(
            ...
            'use_voice_recognition' => $optVoiceRecognition,
    ...
    function get_access_modes($username) {
        $this->db->select('*');
        $this->db->from('use_access_mode');
        $this->db->where('use_username', $username);
        $query = $this->db->get();
        return $query->result();
    }
