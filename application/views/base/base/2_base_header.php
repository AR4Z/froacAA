<header id="unalTop" role="navigation" aria-label="Barra de navegación Universidad Nacional de Colombia">
  ...
  <div class="seal">
    ...
    <select id="selectLanguage" onchange="javascript:window.location.href='<?php echo base_url(); ?>LanguageSwitcher/switchLang/'+this.value;">
      <option data-content='<span class="flag-icon flag-icon-co"></span> Español' value="spanish" <?php if (
                                                                                                    $this->session->userdata('site_lang')
                                                                                                    == 'spanish'
                                                                                                  ) echo 'selected="selected"'; ?>>Español</option>
      <option data-content='<span class="flag-icon flag-icon-us"></span> Inglés' value="english" <?php if (
                                                                                                    $this->session->userdata('site_lang')
                                                                                                    == 'english'
                                                                                                  ) echo 'selected="selected"'; ?>>Inglés</option>
      <option data-content='<span class="flag-icon flag-icon-br"></span> Portugués' value="portuguese" <?php if (
                                                                                                          $this->session->userdata('site_lang')
                                                                                                          == 'portuguese'
                                                                                                        ) echo 'selected="selected"'; ?>>Portugués</option>
    </select>
...
<?php if (!($this->session->userdata('logged_in')) || $this->session->userdata('need_screen_reader') || $this->session->userdata('need_custom_interfaz') || $this->session->userdata('need_narrator')  || $this->session->userdata('need_lsc_translator') || $this->session->userdata('need_structural_nav') || $this->session->userdata('need_virtual_keyboard')) : ?>
  ...
  <div class="btn-group btn-group-toggle" style="position:absolute;bottom:5px;right:15px;margin:0;padding:5px 3px;z-index:10000;" data-toggle="buttons">
    <a id="accessibilityBarButton" class="btn btn-success" data-toggle="collapse" href="#accessibilityBar" role="button" aria-expanded="false" aria-controls="accessibilityBar">
