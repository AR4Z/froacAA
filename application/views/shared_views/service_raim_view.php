...
if($_GET){
    $tabla = array_key_exists('raim', $_GET) ? $_GET['raim'] : null;
...
function get_oas_bget($orParams, $andParams) {
    ...
    $dbconn3 = pg_connect("host='localhost' port='5432' dbname='masle' user='postgres' password='luis'");
    ...
    $andQuery = pg_fetch_all(pg_query(#$this->db->query(
        ...
        WHERE query @@ campo_busqueda_index_col and lo_location is not null and rep_id='41'
        ORDER BY rank DESC;"
    ...
    $orQuery =  pg_fetch_all(pg_query(#$this->db->query(
        ...
        and rep_id='41' ORDER BY rank DESC;"
